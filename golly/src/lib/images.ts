...
export async function uploadActivityImages(
  ...
  if (args.heroImage) {
    ...
    const { error } = await supabase.storage
      .from(bucket)
      .upload(path, args.heroImage, { upsert: true, cacheControl: "3600" });
  ...
  for (let i = 0; i < args.galleryImages.length; i++) {
    ...
    if (error) {
      console.error(`[uploadActivityImages] gallery image ${i} failed:`, error);
      anyFailed = true;
  ...
  return { heroUrl, galleryUrls, anyFailed };
}
...
export function getHeroImage(camp?: CampImageFields | null): string | null {
  if (!camp) return null;
...
export function getGalleryImages(
  ...
  opts?: { includeHero?: boolean; max?: number },
): string[] {
  if (!camp) return [];
