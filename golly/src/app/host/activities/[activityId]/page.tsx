...
export default function ActivityDetailPage() {
  ...
  useEffect(() => {
    ...
    const loadGuests = async () => {
      ...
      const { data } = await supabase.from("camp_bookings")
        .select(`id, camp_id, parent_id, child_id, status, created_at, child:children!camp_bookings_child_id_fkey(id, legal_name, preferred_name, birthdate, age_years, avatar_emoji)`)
        .eq("camp_id", activity.id).not("child_id", "is", null).order("created_at", { ascending: false }).returns<CampBookingRow[]>();
      if (!alive) return;
      setGuests(data ?? []);
      setGuestsLoading(false);
    };
    const loadCount = async () => {
      const { count } = await supabase.from("camp_bookings").select("id", { count: "exact", head: true }).eq("camp_id", activity.id).neq("status", "declined");
  ...
  const updateGuestStatus = async (bookingId: string, status: BookingStatus) => {
    ...
    const { error } = await supabase.from("camp_bookings").update({ status, updated_at: new Date().toISOString() }).eq("id", bookingId);
                ...
                return (
                  <div key={g.id} className="flex w-full items-center justify-between rounded-2xl bg-card px-4 py-3 hover:bg-muted/40 transition-colors">
                    <Link href={`/host/activities/${activityId}/guests/${g.id}`} className="flex items-center gap-3 flex-1 min-w-0">
                      <div className="h-7 w-7 rounded-full bg-amber-100 flex items-center justify-center text-[13px] shrink-0">{emoji}</div>
