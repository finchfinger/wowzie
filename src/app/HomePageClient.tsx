...
import { type CampRow, dedupeByProgram } from "@/lib/camp-utils";
...
const FEATURED_HERO_CAMPS: Array<{ name: string; slug: string; short_id: string | null | undefined; image: string }> = [
  { name: "Kids at camp having fun", slug: "", short_id: null, image: "/images/home-hero-kids.jpg" },
];
...
type Props = { initialPool: CampRow[] };

export function HomePageClient({ initialPool }: Props) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => setHeroIndex((i) => (i + 1) % heroCamps.length), 8000);
  ...
  const gridItems = useMemo(() => {
    ...
    if (sortMode === "featured") {
      ...
      modePool.sort((a, b) => (ts(b.created_at) ?? -1) - (ts(a.created_at) ?? -1));
    } else if (sortMode === "new") {
      const windowMs = 45 * 24 * 60 * 60 * 1000;
      const now = Date.now();
      modePool = pool.filter((c) => { const t = ts(c.created_at); return t != null && now - t <= windowMs; });
      modePool.sort((a, b) => (ts(b.created_at) ?? -1) - (ts(a.created_at) ?? -1));
    } else {
    ...
    if (chosen.length < LIMIT) {
      const fillPool = pool
        ...
        .sort((a, b) => (ts(b.created_at) ?? -1) - (ts(a.created_at) ?? -1));
                ...
                <form onSubmit={onHeroSearch} className="pt-2 space-y-3">
                            ...
                            <>
                              ...
                              <div className="px-4 py-1">
                                <div className="flex items-center justify-between mb-1">
                                  ...
                                  <button type="button" onMouseDown={clearRecentSearches} className="text-xs text-muted-foreground hover:text-foreground transition-colors">Clear</button>
                                </div>
                  ...
                  <div className="hidden sm:grid grid-cols-1 sm:grid-cols-3 gap-2">
                    ...
                    <Popover open={datesOpen} onOpenChange={setDatesOpen}>
                      ...
                      <PopoverContent align="start" side="bottom" sideOffset={8} className="w-[min(560px,calc(100vw-2rem))] p-3">
                        <div className="mt-1">
                          <Calendar
                            mode="range"
