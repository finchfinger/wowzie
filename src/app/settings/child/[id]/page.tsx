...
export default function ChildDetailPage() {
  ...
  const handleDelete = async () => {
    if (!child) return;
    const ok = window.confirm("Remove this child from your Wowzi account? This cannot be undone.");
    if (!ok) return;
    const { data: userData } = await supabase.auth.getUser();
        ...
        <form className="space-y-5" onSubmit={(e) => { e.preventDefault(); void saveChild(); }}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <p className="mt-1 text-[11px] text-muted-foreground">Age shown on Wowzi is calculated from this date.</p>
            </div>
            <div>
          ...
          <div>
            ...
            <p className="mt-2 text-[11px] text-muted-foreground">
              This helps Wowzi recommend better camps and helps hosts prepare.
            </p>
          </div>
