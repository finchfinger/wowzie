...
export default function HostApplyPage() {
  ...
  // Pre-fill email + name from signed-in account on first load
  useEffect(() => {
    if (!user) return;
    if (user.email && !emailAddress) setEmailAddress(user.email);

    // Fetch profile to pre-populate name fields
    const prefill = async () => {
      ...
      const legalName: string = (data as any).legal_name?.trim() ?? "";
      const preferredFirst: string = (data as any).preferred_first_name?.trim() ?? "";

      if (legalName || preferredFirst) {
        // Derive first / last from legal_name (split on first space)
        const spaceIdx = legalName.indexOf(" ");
        const derivedFirst = spaceIdx > 0 ? legalName.slice(0, spaceIdx) : legalName;
        const derivedLast  = spaceIdx > 0 ? legalName.slice(spaceIdx + 1) : "";

        if (!firstName) setFirstName(preferredFirst || derivedFirst);
        if (!lastName && derivedLast)  setLastName(derivedLast);
      }
    };
    void prefill();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const bioText = accountType === "individual" ? aboutIndividual.trim() : aboutOrg.trim();
      const { error: upsertErr } = await supabase.from("host_profiles").upsert(
        {
          ...
          applied_at: nowIso,
          updated_at: nowIso,
          settings: { application_notes: lines.join("\n") },
        },
        { onConflict: "user_id" },
      );
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <button type="submit" disabled={submitting} className="rounded-md bg-foreground px-5 py-2 text-sm font-medium text-background hover:opacity-90 disabled:opacity-60">
            ...
            <div className="px-6 pt-8 pb-6">
              ...
              <div className="max-h-[60vh] overflow-auto space-y-4 text-sm text-foreground">
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-muted-foreground text-xs leading-relaxed">Hosts must follow Wowzi's child safety guidelines at all times, including appropriate supervision ratios, mandatory reporting obligations, and our communication policies. Any violations may result in immediate suspension.</p>
                </div>
                <div className="space-y-1.5">
                  <h3 className="font-semibold text-sm">Payments and cancellations</h3>
                  <p className="text-muted-foreground text-xs leading-relaxed">Wowzi collects payment on your behalf and disburses funds according to our payout schedule. Cancellation policies you set are binding. Wowzi's service fee is deducted from each booking.</p>
                </div>
                <div className="space-y-1.5">
                  <h3 className="font-semibold text-sm">Privacy and data</h3>
                  <p className="text-muted-foreground text-xs leading-relaxed">Family contact information shared with you through Wowzi may only be used to coordinate the activity booked. You may not use this data for marketing or share it with third parties.</p>
                </div>
