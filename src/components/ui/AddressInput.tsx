...
export type AddressSelection = {
  ...
  line1?: string;
  city?: string;
  state?: string; // abbreviation like IL
  postalCode?: string;
  country?: string;
...
> & {
  ...
  // Restrict to a country (default US)
  country?: string;

  // Pass through to Input styling
  error?: boolean;
};
...
export const AddressInput: React.FC<AddressInputProps> = ({
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        ...
        // Optional: bias toward cities when mode="city"
        // Keeping this minimal for reliability, but you can tighten later if you want:
        // if (mode === "city") request.includedPrimaryTypes = ["locality"];
        ...
        if (activeRequestIdRef.current !== requestId) return;
        console.warn("[AddressInput] fetchAutocompleteSuggestions failed:", e);
        setSuggestions([]);
        setOpen(false);
      } finally {
  ...
  const handlePick = async (s: any) => {
    try {
      ...
      const state = getComponent(comps, "administrative_area_level_1", true);
  ...
  return (
    <div className="relative">
      ...
      {open && !disabled && suggestions.length > 0 && (
        <div
          className="absolute z-50 mt-2 w-full overflow-hidden rounded-xl border border-black/10 bg-white shadow-lg"
          role="listbox"
        >
